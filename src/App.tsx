...
function App() {
    ...
    const [fileType, setFileType] = useState<"image" | "docx" | "pdf" | null>(
        null,
    );
    ...
    useEffect(() => {
        invoke<string[]>("list_models")
            .then((list) => {
                ...
                log.info(`[前端] 可用模型: ${list.join(", ")}`);
            })
            .catch((err) => {
                log.error(`[前端] 获取模型列表失败: ${err}`);
            });
    }, []);
    ...
    const handleModelChange = async (variant: string) => {
        ...
        try {
            ...
            log.info(`[前端] 切换模型: ${variant}`);
        } catch (err) {
            log.error(`[前端] 切换模型失败: ${err}`);
            alert("切换模型失败: " + err);
        }
    };
    ...
    const handleFileSelect = async () => {
        ...
        try {
            ...
            log.info(`[前端] 读取文件: ${name}`);

            if (name.endsWith(".pdf")) {
                setFileType("pdf");
                setPdfData(dataUrl);
                // 获取 PDF 页数
                const count = await invoke<number>("pdf_page_count", {
                    data: dataUrl,
                });
                setPageCount(count);
                ...
                // 渲染第一页
                if (count > 0) {
                    const firstPage: PageImage = await invoke(
                        "render_pdf_page",
                        {
                            data: dataUrl,
                            page: 0,
    ...
    useEffect(() => {
        ...
        setDisplaySize({
            w: img.naturalWidth * zoomLevel,
            h: img.naturalHeight * zoomLevel,
        });
    }, [zoomLevel]);
    ...
    const handleZoomDelta = useCallback((delta: number) => {
        ...
        setZoomLevel((z) => {
            ...
            const clamped = Math.max(
                ZOOM_MIN,
                Math.min(ZOOM_MAX, Math.round(next / ZOOM_STEP) * ZOOM_STEP),
            );
    ...
    const switchPage = async (pageIdx: number) => {
        ...
        if (fileType === "pdf") {
            ...
            if (cachedPage) {
                ...
                setLoading(true);
                try {
                    const renderedPage: PageImage = await invoke(
                        "render_pdf_page",
                        {
                            data: pdfData,
                            page: pageIdx,
